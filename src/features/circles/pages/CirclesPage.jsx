...
export default function CirclesPagePro() {
  ...
  // Handle circle created by API-driven form
  const handleCircleCreated = (circleItem) => {
    // circleItem should be the created circle returned by your API (handler returns circle)
    if (!circleItem) return;
    setCircles((prev) => [circleItem, ...prev]);
    setSelectedCircleId(circleItem.circleId);
    setIsNewCircleOpen(false);
  };

  /* -------------------------
     Load circles once (sample)
     ------------------------- */
  useEffect(() => {
    setLoadingCircles(true);
    return fakeLoad(() => {
      setCircles(SAMPLE_CIRCLES);
      setSelectedCircleId(SAMPLE_CIRCLES[0]?.circleId || null);
      setLoadingCircles(false);
    }, 160);
  }, []);

  /* -------------------------
     Load posts when selection changes (sample)
     ------------------------- */
  useEffect(() => {
    if (!selectedCircleId) {
      setPosts([]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 antialiased safe-area-padding font-sans">
            ...
            <div className="bg-white rounded-2xl p-4 sm:p-6 mb-4 sm:mb-6 shadow-lg border border-border-clr">
              <div className="hidden lg:flex items-center justify-between">
                ...
                <div className="hidden sm:flex items-center gap-3">
                  ...
                  <div className="px-3 py-2 rounded-xl bg-gray-100 text-sm text-gray-500">{posts.length} posts</div>
                </div>
              </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              {loadingPosts ? (
                ...
                <div className="bg-white rounded-2xl p-6 sm:p-12 text-center shadow-lg fade-in border border-border-clr">
                  ...
                  <button onClick={openCompose} className="w-full sm:w-auto px-6 py-3 rounded-xl text-white font-semibold shadow-lg transition-all duration-200 hover:shadow-xl hover:scale-105 bg-gradient-to-r from-gradient-primary to-gradient-secondary">
                    Create First Post
                  </button>
                </div>
              ) : (
                posts.map((post, index) => (
                  <MobilePostCard key={post.postId} post={post} onLike={toggleLike} style={{ animationDelay: `${index * 100}ms` }} />
                ))
              )}
            </div>
          </section>
      ...
      <MobileBottomNav onCompose={openCompose} onCircles={() => setIsCirclesDrawerOpen(true)} selectedCircle={selectedCircle} />

      {isComposeOpen && (
        <Model title="Create Post" onClose={closeCompose}>
          <CreatePostForm onCreate={createPost} onCancel={closeCompose} />
        </Model>
      )}

      {isNewCircleOpen && (
        <Model title="Create New Circle" onClose={closeNewCircle}>
          <CreateCircleForm onCreate={handleCircleCreated} onCancel={closeNewCircle} />
        </Model>
      )}
    </div>
  );
